<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <title>Clean Odisha</title>
    <style>
        .cnt-1{
            margin-top: 2%;
        }
        .r{
            margin-top: 1%;
        }
        body{
            background-image: url('https://img.freepik.com/free-vector/rubbish-street-scene_1308-40499.jpg?w=1060&t=st=1669403277~exp=1669403877~hmac=b8ab1c9f9524bb00fa89ac7670d6660fc509347facc72fc7e6cfab18180c2afe');
            background-repeat: no-repeat;
            background-size: cover;
        }
        .c1{
            background-color:rgba(0, 0, 0, 0.1);
            
        }
        li .a1{
            color: aliceblue;
        }
    </style>
  </head>
  <body>
    <div class="container-fluid">
       <nav class="navbar navbar-expand-lg navbar-yellow bg-dark">
            <div class="container-fluid">
                <a class="navbar-brand" href="#">
                <img src="/docs/5.0/assets/brand/bootstrap-logo.svg" alt="" width="30" height="24" class="d-inline-block align-text-top">
                GARBAGE MANAGEMENT
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link active a1" aria-current="page" href="index.php">Home</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link a1" href="#">About Us</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link a1" href="#">Contact Us</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link a1" href="#">Our Mission</a>
                    </li>
            </ul>
          </div>
        </div>
    </nav>
</div>
    
      <div class="container cnt-1">
        <div class="row">
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">PUBLIC</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href="public-login.php" class="btn btn-primary">REGISTER/SIGN IN</a>
                </div>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">MUNICIPALITY</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href=".\muncipalty\officer-login.php" class="btn btn-primary">LOGIN</a>
                </div>
              </div>
            </div>
          </div>
          <div class="row r">
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">HEALTHCARE PROVIDERS</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href="health_care\health_care_login.php" class="btn btn-primary">LOGIN</a>
                </div>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">PRIVATE SECTOR(PPP)</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href="ppp_page/public_sign_in.php" class="btn btn-primary">SIGN IN</a>
                </div>
              </div>
            </div>
          </div>
          <div class="row r">
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">WORKER</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href="#" class="btn btn-primary">LOGIN</a>
                </div>
              </div>
            </div>
            <div class="col-sm-6">
              <div class="card">
                <div class="card-body c1">
                  <h5 class="card-title">GOVERNMENT</h5>
                  <p class="card-text">With supporting text below as a natural lead-in to additional content.</p>
                  <a href="#" class="btn btn-primary">LOGIN</a>
                </div>
              </div>
            </div>
          </div>
      </div>

      

 
  </body>
  
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>

    <!-- Option 2: Separate Popper and Bootstrap JS -->
    <!--
    <script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js" integrity="sha384-IQsoLXl5PILFhosVNubq5LC7Qb9DXgDA9i+tQ8Zj3iwWAwPtgFTxbJ8NT4GN1R8p" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js" integrity="sha384-cVKIPhGWiC2Al4u+LWgxfKTRIcfu0JTxR+EQDz/bgldoEyl4H0zUF0QKbrJ0EcQF" crossorigin="anonymous"></script>
    -->
  </body>
</html>